...
const track = (title) => ({ title, artists: [{ name: "An Artist" }] });
const artist = (name) => ({ name, genres: ["pop"] });
...
test("put is an upsert: same key keeps the id and refreshes the value", async () => {
    const t = setup();
    const key = { kind: "track", provider: "spotify", externalId: "t1" };
...
test("get on an unknown key returns null", async () => {
    const t = setup();
    expect(await t.query(api.example.get, {
        kind: "track",
        provider: "spotify",
        externalId: "missing",
    })).toBeNull();
...
test("getByIsrc returns fresh tracks across providers and excludes expired", async () => {
    ...
    expect(fresh.map((e) => e.provider).sort()).toEqual([
        "apple",
        "spotify",
    ]);
});
test("invalidate deletes an entry (true), and is a no-op on a missing key (false)", async () => {
    const t = setup();
    const key = { kind: "album", provider: "spotify", externalId: "al1" };
    await t.mutation(api.example.put, {
        ...key,
    ...
    expect(await t.mutation(api.example.invalidate, {
        kind: "album",
        provider: "spotify",
        externalId: "nope",
    })).toBe(false);
