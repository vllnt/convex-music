...
- `[planned]` Field-source policy — configure what comes back from search + catalog reads: which
  entity kinds, which fields, and per-field which provider supplies it (e.g. artists with Spotify
  image, tracks with Apple `previewUrl`, or both). Default at mount, override per call.
