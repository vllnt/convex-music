...
- `provider-adapters.3` `planned` — Apple Music adapter — **full V8 re-architecture (not a port)**: songtrivia signs the ES256 JWT with `jsonwebtoken` inside a `"use node"` action (`apple/client.ts:1,64`), which a V8 component **cannot run**. Re-implement with Web Crypto `subtle.sign` (ECDSA P-256): PKCS8 key import + JWS assembly + base64url (`Buffer`→`btoa`/Uint8Array), zero-dep; cache the signed JWT (long TTL — songtrivia uses a 6-month `exp`) via `@convex-dev/action-cache`. Highest-risk adapter — de-risk early.
